        ...
        >
          <td>
            <input
              ...
              v-on:click="addToSavedUserArray($event)"
              v-bind:checked="isChecked(user.id)"
            />
          </td>
...
export default {
  ...
  data() {
    return {
      ...
      selectedUsers: [],
      selectedUserIDs: [],
      //need to disable buttons IF selected user is empty
      showForm: false,
      users: [
        {
          id: 1,
  ...
  methods: {
    ...
    flipStatus(id) {
      const userToChange = this.users[this.findUserIndexById(id)];
      userToChange.status =
        userToChange.status === "Active" ? "Disabled" : "Active";
    ...
    addToSavedUserArray(event) {
      //console.log(event.target.checked);
      // console.log(event.target.id);
      if (event.target.checked == true) {
        this.selectedUserIDs.push(Number.parseInt(event.target.id));
      } else {
        this.selectedUserIDs = this.selectedUserIDs.filter((id) => {
          return Number.parseInt(event.target.id) != id;
        });
      }
    },
    isChecked(id) {
    ...
    enableSelectedUsers() {
      this.selectedUserIDs.forEach((id) => {
        const userIdIndex = this.findUserIndexById(id);
        this.users[userIdIndex].status = "Active";
      });
    },
    disableSelectedUsers() {
      this.selectedUserIDs.forEach((id) => {
        const userIdIndex = this.findUserIndexById(id);
        this.users[userIdIndex].status = "Disabled";
      });
    },
    deleteSelectedUsers() {
