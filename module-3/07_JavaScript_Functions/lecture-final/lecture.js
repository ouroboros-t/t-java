...
/* Using forEach with a named callback function */
...
function printArrayElementsV2(arrToPrint) {
  arrToPrint.forEach(function (val) {
    console.log(val);
  });
}
...
/**
 * Returns a new array containing only the even values in arrToFilter
 * 
 * @param {number[]} [arrToFilter=[]]
 * @returns See above
 */
function evensOnly(arrToFilter = []) {
  //return arrToFilter.filter(isEven);
...
let myCreate = createSentenceFromUser;
...
function sumAllNumbers(numbersToSum) {
  return numbersToSum.reduce((runningSum, curValue) => runningSum + curValue);
}

function sumAllEvenNumber(numbersToSum) {
...
function sumLetters(previousValue, currentValue) {
  console.log(`In sumLetters with ${previousValue} and ${currentValue}`);
...
function countLettersV2(arrayOfWords) {
  return arrayOfWords.reduce((prevValue, curValue) => prevValue + curValue.length, 0);
}
