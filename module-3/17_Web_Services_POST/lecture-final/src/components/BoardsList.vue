...
export default {
  ...
  created() {
    console.log("In BoardsList created()");
    this.retrieveBoards();
  },
  methods: {
    retrieveBoards(boardIdToShow = -1) {
      boardsService.getBoards().then(response => {
        ...
        if (response.status === 200 && response.data.length > 0) {
          if (boardIdToShow == -1) {
            // No board specified. Select the first one
            this.$router.push(`/board/${response.data[0].id}`);
          } else {
            // Show the board passed to this method
            this.$router.push(`/board/${boardIdToShow}`);
          }
        }
      });
    },
    saveNewBoard() {
      console.log("In saveNewBoard()");
      this.isLoading = true;
      let addPromise = boardsService.addBoard(this.newBoard);
      console.log("addPromise: " + JSON.stringify(addPromise));
      addPromise.then((response) => {
        ...
        alert("Caught an error: " + JSON.stringify(error));
      });
