...
--use AS to assign to a data type
SELECT 3+5 AS sum;
...
-- IS NULL / IS NOT NULL boolean that checks for Null
...
SELECT DISTINCT census_region FROM state; --shows HOW census_region is written into the database... 
...
SELECT city_name, area,population, population/100 AS density FROM city WHERE state_abbreviation = 'CA' OR state_abbreviation = 'FL';
...
SELECT park_name, area, area * 1000 * 1000 AS area_in_meters FROM park;
...
//LIKE is case-INSENSITIVE similar to .equalsIgnoreCase


-- The names of all cities that contain a space
SELECT city_name FROM city WHERE city_name LIKE '% %'; -- the space is in between two % wildcards
...
SELECT park_name, date_established FROM park WHERE date_established < '1900-01-01'; -- this is a date literal in the query. 
