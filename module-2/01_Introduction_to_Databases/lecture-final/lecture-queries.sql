...
SELECT city_name, area FROM city WHERE area < 25.0;


-- The names from all records in the state table that have no assigned census region
SELECT state_name FROM state WHERE census_region IS NULL; -- Nothing equals NULL, including NULL. In other words NULL doesn't equal NULL


-- The names and census regions from all records in the state table that have an assigned census region
SELECT state_name, census_region FROM state WHERE census_region IS NOT NULL; -- Nothing equals NULL, including NULL. In other words NULL doesn't equal NULL
...
SELECT DISTINCT census_region FROM state; -- Returns each unique row from the row set
...
--SELECT city_name FROM city WHERE city_name ILIKE 'a%'; -- ILIKE is a case-insensitive LIKE
