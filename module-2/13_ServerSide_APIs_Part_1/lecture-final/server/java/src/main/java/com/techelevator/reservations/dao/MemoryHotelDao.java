...
public class MemoryHotelDao implements HotelDao {
    ...
    @Override
    public List<Hotel> find(String city, String state) {
        List<Hotel> matchingHotels = new ArrayList<>();

        for (Hotel hotel : hotels) {
            Address curAddress = hotel.getAddress();
            if (curAddress.getState().equalsIgnoreCase(state) &&
                    (city == null || curAddress.getCity().equalsIgnoreCase(city))) {
